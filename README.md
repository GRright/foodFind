...
>
> **当前版本**: v2.9.0 · **家庭数据同步完善版** · **最后更新**: 2026-04-25

---

## ✨ 项目亮点

- 🎯 **智能推荐**：基于营养均衡算法 + 偏好学习 + 季节时令的个性化菜谱推荐
- 👥 **双人协作**：云端配对系统，跨设备实时同步菜谱决策
- 🏠 **家庭群组**：支持多人家庭/群组模式，共享购物清单和打卡监督
- 🔄 **数据同步**：共享菜谱模式、打卡状态、通知系统云端实时同步
- 🔔 **家庭通知**：购物变动、打卡状态、菜谱点赞实时通知
- 🛒 **智能购物清单**：根据菜谱自动生成采购清单，分类整理，支持家庭共享
- 📷 **双模式切换**：做饭模式（菜谱推荐）+ 不做饭模式（美食拍照分享）
- 👍 **喜欢/不喜欢**：菜品详情页一键标记，影响后续推荐排序
- 📤 **好友菜谱分享**：一键分享到好友/朋友圈，查看人数统计
- ⏰ **智能提醒**：饭点打卡提醒、购物清单提醒、特别日子提前提醒
- 📊 **数据可视化**：周报/月报切换、营养趋势图、打卡成就系统
- 🔐 **微信登录**：首次启动自动弹窗授权，获取昵称和头像
- ⚙️ **用餐配置**：工作日/周末分别配置餐次，实时生效
...
│   │   └── friend-menu/friend-menu.vue      # 📤 好友的菜谱
│   │
│   ├── utils/                    # 工具模块
...
**目标**：优化图表交互、修复家庭群组逻辑、提升视觉体验

- [x] **营养摄入趋势图优化**
  - [x] 以今天为中心显示柱状图，提升数据可读性
  - [x] 支持周报/月报模式自适应（80rpx/60rpx柱宽）
  - [x] 独立滚动控制，避免与互动趋势图冲突
- [x] **家庭群组昵称问题修复**
  - [x] 新增 `hasOtherFamilyMembers` 计算属性精确判断家庭成员
  - [x] 仅当存在真正的其他成员时才显示打卡提示
  - [x] 解决"自己和自己打卡"的重复提示问题
- [x] **我的页面图标配置**
  - [x] 为「我喜欢的」配置喜欢图标
  - [x] 为「我的信息」配置我的信息图标
  - [x] 为「偏好设置」配置设置图标
  - [x] 为「关于我们」配置关于图标
  - [x] 新增 `.menu-icon-img` 样式（44rpx × 44rpx）
- [x] **健康标签优化**
  - [x] 将「我的健康标签」改为「家庭健康标签」
  - [x] 允许不选择任何健康标签（表示无过敏源/无健康问题）
  - [x] 优化错误提示信息，更明确地引导用户
