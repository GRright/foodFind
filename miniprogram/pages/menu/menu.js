...
Page({
  ...
  getBalancedRecipes(recipes, count) {
    ...
    const selectedMeat = this.getRandomRecipes(meatRecipes, meatCount);
    const selectedVeg = this.getRandomRecipes(vegRecipes, vegCount);
    
    return [...selectedMeat, ...selectedVeg].sort(() => Math.random() - 0.5);
  },
  ...
  getSampleMenu() {
    ...
    this.data.weekDays.forEach(day => {
      ...
      if (day.date === today) {
        breakfast = this.getRandomRecipes(this.data.allRecipes.breakfast, recipeCount);
        lunch = this.getBalancedRecipes(this.data.allRecipes.lunch, recipeCount);
        dinner = this.getBalancedRecipes(this.data.allRecipes.dinner, recipeCount);
        
        const app = getApp();
        if (app.globalData) {
          app.globalData.dailyMeals = { breakfast, lunch, dinner };
        }
      } else {
        breakfast = this.getRandomRecipes(this.data.allRecipes.breakfast, recipeCount);
        lunch = this.getBalancedRecipes(this.data.allRecipes.lunch, recipeCount);
  ...
  updateMealSections() {
    ...
    const mealSections = this.data.mealSections.map(section => {
      const menus = dayMenu.filter(m => m.meal_type === section.type);
      const menu = menus.length > 0 ? menus[0] : null;
      const nutrition = menu && menu.nutrition ? menu.nutrition : null;
